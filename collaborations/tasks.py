...
from accounts.email_messages import sendEventClosedNotification, sendEventNotification, sendTenderEmailNotification
    ...
    if CompanyEvent.objects.exists():
        ...
        for event in open_events: 
            if today >= event.end_date.date():
                ...
                event.status = "Closed"
                event.save() 
                sendEventClosedNotification(event)
    ...
    if EventParticipants.objects.exists():
        ...
        for participant in event_participants:
            if participant.notify_on <= today: #lesser than is used, because if we check event participation notification once a day and 
                ...
                if sendEventNotification(participant):
...
def check_tender_startdate():
    ...
    if Tender.objects.exists():
        ...
        print("Finished automatic Tender Opening process for ", timezone.now().date())
    else:
